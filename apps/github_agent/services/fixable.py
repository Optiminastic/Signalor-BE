...
# GEO-signal tasks (source=geo_signal, generated by analyzer/services/geo_tasks.py)
# that are inherently OFF-SITE outreach - earning a mention/listing/placement on a
# third-party site - not a repo edit. Without this, the deny-list gate below would
# offer them to the code agent, which then wastes an LLM round-trip only to refuse
# ("requires external outreach... not code changes"), leaving a misleading auto-fix
# panel on a task that can only be done manually.
GEO_OFFSITE_FINDINGS = {
    "geo_citation_gap",  # "Get mentioned on <external domain>"
}
NOT_CODE_FIXABLE: set[str] = set(OFFPAGE_FINDINGS) | INFRA_NOT_FIXABLE | GEO_OFFSITE_FINDINGS

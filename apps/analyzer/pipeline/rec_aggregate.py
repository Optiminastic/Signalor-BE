...
def _severity(rec: dict) -> int:
    """Higher = more severe. Honest ordering input (no fabricated points)."""
    return {"critical": 3, "high": 2, "medium": 1}.get(rec.get("priority", "low"), 0)
...
def dedupe_recommendations(tagged_recs: list[dict]) -> list[dict]:
    ...
    "Strongest" = highest priority severity, tie-broken by richer evidence. The
    ...
    for code in order:
        ...
        winner = max(
            members,
            key=lambda r: (_severity(r), _evidence_richness(r)),
        )
